...
export function buildPrompt(transcript: string, timelineContext = "", energyContext = "", languageContext = "", creatorBrainContext = ""): string {
...
export function buildAdvancedPrompt(transcript: string, timelineContext = "", energyContext = "", languageContext = "", creatorBrainContext = ""): string {
  const variation = pickVariation();
  const tiktokOpener = pickRandom(TIKTOK_OPENING_LINES);
  const tiktokClosing = pickRandom(TIKTOK_CLOSING_LINES);
  const context = buildPromptContext(variation.angle);
